...
const Navbar = () => {
  ...
  useGSAP(() => {
    ...
    if (!isMenuOpen) {
      if (isScrolled) {
        // Desktop: Ẩn Nav Links, hiện Menu Icon
        tl.to(navLinksRef.current, {
          y: -30, opacity: 0, duration: 0.4, ease: "power3.out", pointerEvents: "none"
        }, 0);
        
        // Cả Mobile & Desktop: Contact trượt sang trái
        tl.to(contactRef.current, {
          x: isMobile ? -40 : -60, // Mobile trượt ít hơn
          duration: 0.4, ease: "power3.out"
        }, 0);

        // Hiện Menu Icon (Hamburger)
        ...
        // Trên Desktop thì ẩn Menu Icon khi chưa scroll
        // Trên Mobile thì LUÔN HIỆN Menu Icon nên không ẩn nó đi hoàn toàn
        if (!isMobile) {
            tl.to(menuIconRef.current, {
            x: 20, autoAlpha: 0, duration: 0.3, ease: "power3.in", pointerEvents: "none"
            }, 0);
        } else {
             // Mobile: Luôn giữ Menu Icon hiện
             gsap.set(menuIconRef.current, { autoAlpha: 1, x: 0, pointerEvents: "auto" });
        }
      }
    }
  }, [isScrolled, isMenuOpen]);

  // Animation Mở/Đóng Overlay Menu (Giữ nguyên)
  useGSAP(() => {
    const tl = gsap.timeline();
  ...
  return (
    <>
      <nav 
        ref={containerRef} 
        className="fixed top-0 left-0 w-full flex justify-between items-center px-6 py-4 md:px-12 z-[100] text-white mix-blend-difference font-sans"
      >
        {/* LOGO */}
        <div 
            className="flex items-start gap-2 md:gap-3 cursor-pointer select-none z-[100]"
        ...
        >
          ...
          <div className="flex flex-col items-start">
            <span className="text-[0.5rem] md:text-[0.6rem] font-normal tracking-[0.15em] leading-none lowercase mt-[10px] md:mt-[17px]">algo</span>
            <span className="text-[0.5rem] md:text-[0.6rem] font-normal tracking-[0.15em] leading-none lowercase mt-[2px]">rithm</span>
            <span className="text-[0.5rem] md:text-[0.6rem] font-normal tracking-[0.15em] leading-none lowercase mt-[2px]">base</span>
          </div>
        </div>
        ...
        <div className="flex items-center relative z-[100]">
          ...
          {/* CONTACT BUTTON: Luôn hiện (flex), chỉ ẩn khi vào chế độ mobile quá nhỏ nếu muốn */}
          <div 
            ref={contactRef} 
            className="flex items-center cursor-pointer group relative overflow-hidden w-[100px] md:w-[140px] h-6"
          ...
          >
              ...
              <div className="flex items-center h-full gap-[4px] transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:translate-x-[24px]">
                <span className="text-[10px] md:text-xs font-bold tracking-widest uppercase">Contact</span>
          ...
          {/* MENU ICON (HAMBURGER): Mặc định ẩn trên Desktop khi chưa scroll, Luôn hiện trên Mobile (CSS xử lý opacity ban đầu) */}
          <div 
            ref={menuIconRef} 
            onClick={() => setIsMenuOpen(true)}
            className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col items-start gap-[5px] md:gap-[6px] cursor-pointer hover:opacity-70 transition-opacity md:invisible md:opacity-0"
          >
            <div className="w-[20px] md:w-[26px] h-[2px] bg-white"></div>
            <div className="w-[14px] md:w-[18px] h-[2px] bg-white"></div>
          </div>
      ...
      {/* OVERLAY MENU (GIỮ NGUYÊN) */}
      <div 
        ref={overlayRef}
        className="fixed inset-0 w-screen h-screen bg-black z-[90] flex flex-col justify-center items-end pr-6 md:pr-12 pt-24 opacity-0 scale-0 pointer-events-none origin-center"
      ...
      >
        <div ref={overlayLinksRef} className="flex flex-col gap-2 text-right">
            <div 
              ...
              onClick={() => handleScrollTo('home')}
            >
              <div className="opacity-0 -translate-x-8 transition-all duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:opacity-100 group-hover:translate-x-0 hidden md:block">
                <ArrowIcon className="w-16 h-16 md:w-24 md:h-24 text-white" />
              </div>
              <h1 className="text-5xl md:text-8xl font-bold tracking-tighter text-[#333] transition-all duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:text-white group-hover:translate-x-2">
                HOME
              </h1>
            </div>
